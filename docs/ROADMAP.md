...
- Token auth (long-lived per-device tokens stored in `MobileDevice` model with revoke-on-demand). *(API plan doc shipped v3.17.345; auth endpoints v3.17.346; dashboard + organizations v3.17.347; assets v3.17.348; tickets v3.17.349; KB endpoints — list / search / detail with raw markdown + rendered HTML — shipped v3.17.350; vault / monitoring / profile endpoints land v3.17.351 → v3.17.353; mobile-client login body field-name mismatch fixed v3.17.446 — was sending `email`, server reads `username`)*
